...
// Double check this path! If AuthModal is in app/components/AuthModal.jsx, use:
...
export default function ServicesPage() {
  ...
  return (
    // Added relative and z-0 to ensure the page container doesn't overlap the fixed modal
    <div className="relative z-0 min-h-screen bg-[#0a0a0a] text-white font-sans selection:bg-rose-600">
      
      {/* 
         CRITICAL: The AuthModal is fixed position. 
         Ensure its z-index inside AuthModal.jsx is higher than 100 (e.g., z-[999])
      */}
      <AuthModal 
        modalOpen={modalOpen} 
              ...
              <motion.div
                ...
                onClick={async () => {
                  ...
                  if (!session) {
                    ...
                    // 2. Open Modal with a tiny delay to ensure smooth transition
                    setIsRegister(false);
                    setTimeout(() => {
                        setModalOpen(true);
                    }, 100);
                ...
                <div className="relative z-10 h-full p-10 flex flex-col justify-between">
                  <div className="flex justify-between items-start">
                    ...
                    <div className="w-10 h-10 rounded-full border border-white/10 flex items-center justify-center group-hover:border-rose-600/50 group-hover:bg-rose-600 transition-all duration-500">
                      <ArrowUpRight size={18} className="group-hover:rotate-45 transition-transform text-white" />
                    </div>
                  </div>

                  <div>
                    <h3 className="text-3xl font-black uppercase tracking-tighter mb-4 leading-none group-hover:text-rose-500 transition-colors duration-300 text-white">
                      {service.name}
                    </h3>
